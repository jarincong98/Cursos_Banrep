...
(C-psi_l*L^(1+eta)/(1+eta))^(-sigma) = beta*(C(+1)-psi_l*L(+1)^(1+eta)/(1+eta))^(-sigma)*(1+R_star(+1));
...
(C-psi_l*L^(1+eta)/(1+eta))^(-sigma)*(1+phi_k*(K-K(-1))) = beta*(C(+1)-psi_l*L(+1)^(1+eta)/(1+eta))^(-sigma)*(1-delta + alpha*Y(+1)/K - phi_k*(K(+1)-K));
